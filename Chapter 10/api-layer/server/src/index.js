...
const getQuotesByCursor = async (cursor, limit) => {
  ...
  return { quotes, nextCursor: endIndex < quotesData.quotes.length - 1 ? endIndex + 1 : null };
};
...
app.get("/", async (request, response) => {
  try {
    ...
    if (!page && !cursor) throw new Error('Missing parameters. Please provide "page" or "cursor" in the request query.');
    await sleep();
    const limit = 5;
    if (page) return response.json(await getQuotesByPage(parseInt(page), limit));
    if (cursor) return response.json(await getQuotesByCursor(parseInt(cursor), limit));
  } catch (error) {
    response.status(500).json({ error: 'Missing parameters("page" or "cursor"). ' });
  }
});
